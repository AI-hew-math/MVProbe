...
from utils import fix_seeds, str2bool, imagenet_ids_to_class_names, get_class_ids_from_hf_dataset, get_class_ids_from_directory
...
def compute_accuracy(logits, labels):
    ...
    acc = correct[:1].reshape(-1).float().sum(0).item()
    return acc
...
def setup_training(args, device, clip_model):
    ...
    args.layer_shape = input_shape

    model = probex.ProbeXZeroshot(
        input_shape=input_shape, n_probes=args.n_probes, proj_dim=args.proj_dim, rep_dim=args.rep_dim, clip_dim=args.clip_dim,
...
def run_eval(args, model, loader, clip_features, criterion, device):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_samples = 0
    ...
    with torch.no_grad():
        for B, A, y_text, y_label in loader:
            ...
            logits = model(inputs, clip_features, active_branches=active_branches)
...
def run_train(args, model, loaders, clip_features, criterion, optimizer, device):
    ...
    for epoch in tqdm(range(args.n_epochs)):
        ...
        for B, A, y_text, y_label in train_loader:
            ...
            optimizer.zero_grad()
            logits = model(inputs, train_clip_features, active_branches=active_branches)
            loss = criterion(logits, y_label)
        ...
        if epoch % args.eval_every == 0:
            ...
            val_loss, val_accuracy = run_eval(args, model, val_loader, train_clip_features, criterion, device)
            ...
            val_holdout_loss, val_holdout_accuracy = run_eval(args, model, val_holdout_loader, holdout_clip_features, criterion, device)
            ...
            test_loss, test_accuracy = run_eval(args, model, loaders['test'], train_clip_features, criterion, device)
            test_ho_loss, test_ho_accuracy = run_eval(args, model, loaders['test_holdout'], holdout_clip_features, criterion, device)
            print(f"  Test Acc: {test_accuracy:.2f}%, Test Holdout Acc: {test_ho_accuracy:.2f}%")
...
def final_evaluation(args, model, loaders, clip_features, criterion, device):
    ...
    _, results['best_val_train_acc'] = run_eval(args, model, train_loader, train_clip_features, criterion, device)
    _, results['best_val_val_acc'] = run_eval(args, model, val_loader, train_clip_features, criterion, device)
    _, results['best_val_test_acc'] = run_eval(args, model, test_loader, train_clip_features, criterion, device)
    _, results['best_val_val_holdout_acc'] = run_eval(args, model, val_holdout_loader, holdout_clip_features, criterion, device)
    _, results['best_val_test_holdout_acc'] = run_eval(args, model, test_holdout_loader, holdout_clip_features, criterion, device)
    ...
    _, results['best_holdout_train_acc'] = run_eval(args, model, train_loader, train_clip_features, criterion, device)
    _, results['best_holdout_val_acc'] = run_eval(args, model, val_loader, train_clip_features, criterion, device)
    _, results['best_holdout_test_acc'] = run_eval(args, model, test_loader, train_clip_features, criterion, device)
    _, results['best_holdout_val_holdout_acc'] = run_eval(args, model, val_holdout_loader, holdout_clip_features, criterion, device)
    _, results['best_holdout_test_holdout_acc'] = run_eval(args, model, test_holdout_loader, holdout_clip_features, criterion, device)
...
def train_layer(args, device, clip_model):
    ...
    # Cleanup: delete checkpoint files after final evaluation
    if args.cleanup_checkpoints:
        for ckpt_path in [args.checkpoint_path_val, args.checkpoint_path_val_holdout]:
            try:
                if os.path.isfile(ckpt_path):
                    os.remove(ckpt_path)
            except Exception as e:
                print(f"[WARN] Failed to remove checkpoint {ckpt_path}: {e}")
...
def default_argument_parser():
    ...
    parser.add_argument('--training_lr', type=float, default=3e-4, help='The learning rate for the model')
    parser.add_argument('--batch_size', type=int, default=128, help='The batch size for the model')
    parser.add_argument('--n_epochs', type=int, default=400, help='The number of epochs for the model')
    ...
    parser.add_argument('--cleanup_checkpoints', type=str2bool, default=False,
                        help="After final eval, delete checkpoint files to save disk.")

    return parser


def setup_paths(args):
    args.output_path_root = f"{args.output_path}/ProbeX/abl-{args.ablation}/n_probes-{args.n_probes}/proj_dim-{args.proj_dim}/rep_dim-{args.rep_dim}"
